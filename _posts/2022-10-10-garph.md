...
即：在访问完**当前节点**后，首先访问**当前节点的第一个邻接节点**。

可以看到：这样的访问策略是优先往**纵向深入**，而 不是 对一个节点的所有邻接节点进行 横向访问。显然深度优先搜索是一个**递归**过程。

不够清楚的话，考虑三个节点 A，B，C ，我们可以 A - B ，B - C （纵向深入，一条路跑到黑），也可以 A - B， A - C （先可着一个节点来，横向满足）。显然 DFS 指的是前者。
...
添加节点的顺序为 A、B、C、D、E ，那么 A 为第一个初始节点；

![dfs03.png](/images/dfs03.png "DFS"){: .align-center}

![dfs04.png](/images/dfs04.png "DFS"){: .align-center}

![dfs05.png](/images/dfs05.png "DFS"){: .align-center}

![dfs06.png](/images/dfs06.png "DFS"){: .align-center}

![dfs07.png](/images/dfs07.png "DFS"){: .align-center}


关于其中的方法递归，更详细的展示如下。当第一个邻接节点不存在时，需要**回溯**到上一个递归。
...
如果我们改变加入节点的顺序呢？上面是 A-B-C-D-E ，我们看一下 D-A-C-E-B 是怎么走的：
