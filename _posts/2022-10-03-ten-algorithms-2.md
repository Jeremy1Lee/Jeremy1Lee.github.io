...
为什么要有前驱结点？因为当前的道路不一定最短，随时调整，回溯。知道了前驱就能调整访问结点的顺序。比如从 G 点出发，会访问 G,A、G,B、G,E、G,F，那么 A、B、E、F 的前驱节点就是 G。 dis 数组记录**出发点到其他所有顶点的距离**，比如从 G 出发，到过了 G,A ，再从 A,C 到 C，则记录的是 G 到 C 的路径距离。Dij算法就是不停动态更新这三个数组，得到最终结果。
