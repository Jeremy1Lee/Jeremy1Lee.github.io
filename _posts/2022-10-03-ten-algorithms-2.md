...
代码容易，唯一**难点是前驱节点的赋值**。为什么 [i, j] 的前驱节点是 preOrder[k][j] 而不是 k ？看下图：
...
所以，路线 [i,j] 的前驱应该为 离 i 或 j 最近的点，可以写 preOrder[i][j] = preOrder[k][j]，或者preOrder[i][j] = preOrder[i][k]，但是不能写preOrder[i][j] = k 。具体怎么写，还取决于 preOrder 的初始化。代码这样写会写成有向，A到G的前驱是A，G到A的前驱是G，即返回离终点最近的节点。其实可以统一定义一下，即定义是离起点最近还是离终点最近。把路径打印出来也很简单，即起点固定，[i,j]返回 z，[i,z]返回y，[i,y]返回x，直到[i,x]返回i为止，路线呼之欲出，就是j-z-y-x-i。
...
这几个算法都符合动态规划 DP 的思想：用表、数组保存当前最优，步步加入新节点，并更新最优。区别在于一个的状态转移方程非常明确（如01背包），另一个并不明确，要针对新情况手动求解（因为谁也不知道新加入的节点和哪个节点连着，以及连接的距离有多长）。我认为后者更普适，即状态转移的规律更不明显。
