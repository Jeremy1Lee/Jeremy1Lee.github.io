...
- 顶点 vi 到顶点 vk 的最短路径已知为 Lik。并不一定是直连线，更有可能是线段，即此时已知vi 到 vk的最优路径；

- 顶点 vk 到 vj 的最短路径已知为 Lkj，同样不一定直连，只是当前已知最优路径；

- 顶点 vi 到 vj 的路径为 Lij；

![floyd01.png](/images/floyd01.png "简单考虑三个顶点"){: .align-center}

则： vi 到 vj 的最短路径为：<mark>min( (Lik + Lkj)，Lij )</mark>，vk 的取值为图中所有顶点，则可获得 vi 到 vj 的最短路径。注意只是当前最优路径，实际并不一定直接获得，而是通过循环和迭代获得。图解如下：

![floyd01.png](/images/floyd01.png "简单考虑三个顶点"){: .align-center}

Floyd 算法核心是维护两张表，即两个二维数组：**初始各顶点之间距离表** 和 **初始顶点前驱表**。
...
如果对这三个数组遍历，首先出发顶点取 A ，中间顶点取 A ，终点是 A 。出发顶点取 A ，中间顶点取 A ，终点是 B , C, D, E ......，然后出发顶点A，中间节点 B ，终点A, B, C,... ，每遍历一次（做一步）把最短距离都更新到距离矩阵中。

可能你会问，就三层 for ，每次考虑的不就都只是 1-2-3 这种三顶点的简单路径吗？——无需担心这样遍历每次都只是三个节点的简单路径：因为每次遍历记录的是**当前最优路径**。最优路径在遍历伊始很可能是 A - B 这种简单路径，但在深入遍历时如果 A - G - B 更短，它会代替 A-B ，成为此时的最短路径并分别存入【距离表和路径表】，此后如果遍历到路线 A - B - X ，里面的 A-B 取的都是 A - G - B，脱离了简单路径。**即随着遍历深入，路径存在积累，可能会越来越复杂，越来越贴近实际的最短路线。**表面上每次考虑的只是 A-B ，实际则是 A -i-j-k-...- B 。这样全部遍历下来，所有路径都被考虑到了。不过，显然三层 for 循环的时间复杂度较高。

再看看这三个二维数组，有没有想到 Dij 算法？事实上，该二维数组的行/列，恰好就是 Dij 算法的三个一维数组。意义都是当前【出发结点到其他结点的最短距离】和【各结点的前驱结点】，Dij 多了一个已访问节点，是针对单一顶点而言的，Floyd 则是所有节点都要算出来，因此所有都得考虑一遍。
